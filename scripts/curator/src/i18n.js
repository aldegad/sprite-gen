...
const STR = {
  en: {
    ...
    breatheAmp: "amp", breatheCountLabel: "breaths/loop", breatheFitted: (n) => `→ ${n} applied`,
    tBreatheFitted: (len) => `the loop is ${len} frames — the requested count does not divide it, so the nearest dividing count is applied (loop length never changes)`, tBreatheCount: "breath count per loop — free number; a non-divisor is fitted to the nearest count that divides the loop (see the strip caption)", breatheSub: "sub-pixel (experimental)",
  ...
  ko: {
    ...
    breatheAmp: "진폭", breatheCountLabel: "호흡/루프", breatheFitted: (n) => `→ 적용 ${n}회`,
    tBreatheFitted: (len) => `루프가 ${len}프레임이라 요청 횟수가 나눠떨어지지 않아, 가장 가까운 나눠떨어지는 횟수로 적용됩니다 (루프 길이는 불변)`, tBreatheCount: "루프당 호흡 횟수 — 자유 입력. 루프에 나눠떨어지지 않으면 가장 가까운 횟수로 보정 (스트립 캡션에 표시)", breatheSub: "서브픽셀 (실험)",
