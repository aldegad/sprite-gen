...
- The hi-res `orig/` display twin is now baked at a **per-row native scale**:
  ...
  bounded by `2048 // cell` (was a fixed x4 cap). On high-pitch raws (hero_v8
...
- Verified on a hero_v8 clone heal: 11 rows re-derived, final frames
...
- The curator's pixel-grid overlay now always draws the content box divided
  ...
  cuts (`input_grids`) are per-frame non-uniform (spacing 0.5-1.5 on hero_v8
...
- **Per-frame pitch keeps its own truth only inside the strip consensus "pitch
  ...
  Root cause of the hero_v7 heal wipe-out (Soohong 2026-07-22): up_run frame-2
...
- **Per-frame pitch is the primary truth** (Soohong 2026-07-20, plan
  ...
  hero_v7 down_jump frame-0 (consensus 13.00 vs own 12.50) halved the eyes.
...
## v1.56.71 "Atelier" - Gen timeout default 180s
...
## v1.56.70 "Atelier" - Hard generation timeout + one observable retry
...
## v1.56.69 "Atelier" - Anchor truth = curated SEQUENCE HEAD, not index 0 + take-record isolation
...
- Engine fix (root cause of "the side anchor is not my edited image", Soohong
  ...
  pixel ops, pp variant) - verified on the hero run: all 148 side_idle
...
## v1.56.68 "Atelier" - One generation-trigger idiom (invariant refactor)
...
## v1.56.67 "Atelier" - Anchor ref = curated truth, rebaked per generation; honest anchor chip
...
## v1.56.66 "Atelier" - Row Save button: matched height + far-right placement
...
## v1.56.65 "Atelier" - Row reroll button (candidate takes) + selection flip as a tool action
...
## v1.56.64 "Atelier" - Hand-tool pan everywhere + compare checker fix + row Save popover + breathe on/off in the zoom player
...
## v1.56.63 "Atelier" - Engine-owned sidecar fields survive client saves
...
- Fix: a curation save from the webview silently DROPPED per-state fields it
  ...
  (found by darami's integrity audit on the hero run). The server now
...
## v1.56.62 "Atelier" - Orchestrator-neutral skill + troubleshooting doc
...
## v1.56.61 "Atelier" - Child engines spawn with a clean env (nested-agent hang fixed)
...
## v1.56.60 "Atelier" - Save popovers: closed by default, unified everywhere
...
## v1.56.59 "Atelier" - Compare export formats + codified gen concurrency
...
## v1.56.58 "Atelier" - Static-pose recipe codified from human curation data

- New docs/static-pose-recipe.md, derived from Soohong's hand-curated
  hero 3-direction idles ("they are all still idles - sitting, lying,
...
## v1.56.57 "Atelier" - Waist-line breathe default (belt rule + inheritance)
...
## v1.56.56 "Atelier" - Anchor ref truth = curated export (codified)
...
## v1.56.55 "Atelier" - Lazy atlas bake (zero cost while off-screen)
...
## v1.56.54 "Atelier" - Atlas updates live
...
## v1.56.53 "Atelier" - Compare guide controls: hide / show / clear all
...
## v1.56.52 "Atelier" - Shortcuts work in Korean IME / any keyboard layout
...
## v1.56.51 "Atelier" - Pointer hits exactly the pixel under the cursor
...
## v1.56.50 "Atelier" - Frame-space SSoT; eyedropper picks the color you see
...
## v1.56.49 "Atelier" - WYSIWYG editing under transforms, atlas staleness
...
## v1.56.48 "Atelier" - Breathe line lands correctly on cold load
...
## v1.56.47 "Atelier" - Marquee ergonomics, tool hotkeys, auto landmarks
...
## v1.56.46 "Atelier" - Sticky pen color, persistent compare setup, compare GIF
...
## v1.56.45 "Atelier" - Compare canvas: undo/redo, alignment reset, playback
...
## v1.56.44 "Atelier" - Compare canvas v2 + nameable instances
...
## v1.56.43 "Atelier" - Breathe audit: exact breath count + canonical preview
...
## v1.56.42 "Atelier" - Anchor compare canvas, relink, bake-truth breathe line
...
## v1.56.41 "Atelier" - Boundary-snapped sampling (organic grid truth)
...
## v1.56.40 "Atelier" - Duplicates are LINKED by default (one edit truth)
...
## v1.56.39 "Atelier" - Palette lock + frozen rows (approved-art protection)
...
- **Palette lock** — the shared palette is pinned to `palette.lock.json` at
  ...
  composition. `--repalette` explicitly rebuilds+repins. Hero's pin was
...
## v1.56.38 "Atelier" - Correspondence-grid trust gate (harmonic mis-detection)
...
## v1.56.37 "Atelier" - Whole-row fps stepper; per-frame durations withdrawn
...
## v1.56.36 "Atelier" - Per-frame durations, breath stepper, editor speed sync
...
## v1.56.35 "Atelier" - Loop-fit breathing + craft-rule sub-pixel + grid drift fix
...
## v1.56.34 "Atelier" - Seam-band sub-pixel + final-bake filmstrip
...
## v1.56.30 "Atelier" - Breathe editor: Apply button, adjustment undo/redo, load-race fix
...
## v1.56.29 "Atelier" - Curator front-end split into a domain taxonomy
...
## v1.56.28 "Atelier" - Breathe as a row checkbox + multi-line phase cascade
...
## v1.56.27 "Atelier" - Deterministic breathing frames (integer row-shift squash)
...
## v1.56.26 "Atelier" - Base logical image from the TRUE detected grid
...
## v1.56.25 "Atelier" - Full base parity (transforms bake into the file), Aseprite-style palette dock
...
## v1.56.24 "Atelier" - Base editing IS the zoom modal (one component)
...
## v1.56.23 "Atelier" - Base pixel editor with grid snap, curator session hardening
...
## v1.56.22 "Atelier" - fit.conform fully removed (loud rejection)
...
## v1.56.21 "Atelier" - Generative in-betweens (codex/grok), RIFE retired
...
## v1.56.20 "Atelier" - Tween popover on demand + frame-pair pick mode
...
## v1.56.19 "Atelier" - Curator tween button (in-betweens from the GUI)

The interpolation feature surfaces in the curation view (Soohong: "GUI 표시하고").
Full suite **227 passed**; live e2e through the endpoint regenerated the hero_v7
...
## v1.56.18 "Atelier" - AI frame interpolation (RIFE in-betweens as takes)
...
**227 passed**; the hero_v7 blink take regenerated **byte-identical** through the
...
## v1.56.17 "Atelier" - Pixel-perfect fringe crop fix, palette default 48

Two pixel-perfect quality defects found by Soohong while curating hero_v7 at zoom
...
- **Solid-alpha grid crop** (`solid_alpha_bbox`) — `tighten_components` and
  ...
  (hero_v7: bbox +1~4px on most frames). Degenerate all-fringe components fall back
...
- **Run-wide palette default 24 → 48** — the shared median-cut palette is split across
  ...
  batches. Measured on hero_v7's 36-state batch: nearest palette entry to the gold
...
## v1.56.16 "Atelier" - Per-state GIF export, atlas cell reuse, clone origin badges
...
## v1.56.15 "Atelier" - Eyedropper color-sampling tool
...
## v1.56.14 "Atelier" - Curation view matures, generation identity/isolation hardening, per-row preservation, frame clones
...
## v1.56.13 "Forge" - PerfectPixel integration complete, real-generation correction loop proven
...
## v1.56.12 "Forge" - Generation SSoT unified (`sprite_gen/gen/`, perfectpixel-studio C-gen)
...
## v1.56.11 "Edge Runner" - Automatic inspect/score/correction loop (perfectpixel-studio B-loop)
...
## v1.56.10 "Edge Runner" - Run-length-mode pitch estimator (perfectpixel-studio port, cross-check only)
...
accident: on the a game protagonist component, the y pitch collapsed to the x value
...
- **New `crosscheck_pitch_runlen` + a hook right after the pixel-perfect consensus
  ...
  - Axis-ratio (y/x) disagreement > max(2%, 0.7/pitch): the axis-collapse mode — a 3.5%
    ...
    hero_v7's 8-15px band stays silent while the hero-scale (30px) real-collapse
    signal (2.8%) fires.
- **Measured (hero_v7, 22 states, read-only)**: 0 warnings across the 20 healthy
...
## v1.56.9 "Edge Runner" - chroma.mode: ycbcr (perfectpixel-studio port, opt-in)
...
- **Measured (green key)**: on a degraded-source synthetic fixture (a shaded green band
  ...
  on hero_v7's clean flat-key raws (22 states total): fringe (RGB≤150) 0→0 and CbCr
...
## v1.56.8 "Edge Runner" - segmentation: projection (perfectpixel-studio port, opt-in)
...
- hero_v7 measured: zero regression across the 8 carry/action states (natural
...
## v1.56.7 "Edge Runner" - align_x: alpha-centroid (perfectpixel-studio port, opt-in)
...
- **`scripts/measure_align_sigma.py`** — reports the σ of per-frame centroid X for
  ...
  re-extracted). hero_v7 measured: down_run σ 0.53px→0.17px (−68%); down_walk was
...
## v1.56.6 "Edge Runner" - Collapsed frames no longer poison the row consensus pitch + span double-counting

In a game's `down_carry_run`, 3 of 6 frames collapsed to pitch 3.00 (true value
...
## v1.56.5 "Edge Runner" - Axis-mismatch guard: when one axis collapses to a divisor, use the trusted axis

On a game's `down_carry_walk` row, one frame's per-axis pitch came out
...
- **The guard**: if the per-axis pitches diverge by more than 1.5x, the pitch of the
  ...
  rescaling are around 2% (a game chibi base 30.38 / 30.92), so a 3x gap is
...
## v1.56.4 "Edge Runner" - Measure pitch per axis (horizontal/vertical block sizes can differ)

`detect_pixel_grid` forced one pitch onto both axes. Output rescaled at non-uniform
ratios has mismatched horizontal and vertical block sizes — the a game protagonist
...
- The integer scorer is split into `_axis_int_score` / `_axis_int_seed`, shared with
  `detect_pixel_pitch`.
- **Measured (a game chibi base)**: horizontal alignment 11.7% -> 75.7%; vertical
  stays at 92.6%.
...
## v1.56.3 "Edge Runner" - hotfix: even-division grid stretched over bbox remainder
...
## v1.56.2 "Edge Runner" - pixel-grid detection: fractional pitch, phase, divisor collapse

Patch release in the Edge Runner line. Three real bugs in `detect_pixel_pitch` / grid snapping, all found while rebuilding the a game protagonist base and all pinned by synthetic ground-truth tests (`tests/test_pitch_ground_truth.py`).
...
## v1.56.0 "Edge Runner" - Importable package SSoT, behavior unchanged
...
Game-ready pixel-art output for animation rows, built while shipping the a game protagonist (Godot 4, 64px cells). The headline: extraction no longer treats each frame as an independent image — pixel-perfect is a *row* methodology.
