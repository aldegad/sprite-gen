...
생성 생략 계약)를 기록한다. 상세와 좌우 재생성 규칙: [`docs/directional-anchor-workflow.md`](directional-anchor-workflow.md) "Prepare 스캐폴딩".
...
2. Generate one image per state with the engine's own `gen` command (generation is engine-owned; the `image-gen` skill is now a thin shuttle over this — [`docs/gen.md`](gen.md)):
...
Command chain: [`docs/gen.md`](gen.md#provider-topology).
