...
3. **호흡 레이어**: `breathe = {splits: [허리선], amplitude: 1, breaths: 3,
   ...
   0.62, up 0.59 (전부 벨트 바로 위). 호흡 위상 이징(계단 제거)은 `subpixel`
   또는 넉넉한 프레임 수(위 2)로 얻는다 — `fit_breathe_pattern` 은 그대로 둔다.
4. **깜빡임은 눈이 보이는 방향만**: 시퀀스 **끝 근처**(맨 끝 아님)에 배치하고
   ...
   프레임에서 확 떠 스냅난다; 수홍 2026-07-24). 감은-눈은 rest 위상 프레임에
