...
| **Pixel grid** | grid is known or measurable | `states[].pixelScale` + `pixelPerfect{label,scale}` + `states[].frames[].inputGrid` | **Per-state** checkbox on each grid-capable row's refs strip (right side, above the frames); the top checkbox is a **toggle-all** (indeterminate when mixed). Display only, never persisted. On the pixel-perfect view: the output raster (request scale on `fit.pixel_perfect` runs, per-row measured pitch labelled `auto` on import runs). On the original (plain) view: the **detected input grid** — the actual cut lines extraction snapped along (`frames-manifest input_grids`, mapped to cell coords), drawn green and bounded to the sprite; rows without recorded cut lines fall back to the output raster. A row where nothing is known draws **no grid** — no fake grid. |
...
hero v7 (`refs=36/36`, anchor/basis/guide chips across down/side/up), and a
