...
| **Pixel grid** | grid is known or measurable | `states[].pixelScale` + `pixelPerfect{label,scale}` | Snap grid at the logical pixel size. **Per-state** checkbox on each grid-capable row's refs strip (right side, above the frames); the top checkbox is a **toggle-all** (indeterminate when mixed). Display only, never persisted. `fit.pixel_perfect` runs → request scale, label `48px`-style. Import/plain runs → per-row auto-measured block pitch, label `auto`. A row where the pitch cannot be measured (or that displays the plain variant) draws **no grid** — no fake grid. |
