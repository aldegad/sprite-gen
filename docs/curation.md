...
- `pixel_perfect` — **두 층위**. top-level = 런 전체 기본값(웹뷰 우측 상단 "전체 토글" — 모든 줄을 한번에 설정; 줄별 값이 섞이면 웹뷰는 이 필드를 생략한다). `states.<state>.pixel_perfect` = 줄별 override(각 줄 헤더의 토글). 해석 순서는 `curation.frame_variant(curation, state)` 가 SSoT: 줄별 값 > top-level > 기본 `true`. `false` 인 줄은 compose/export/GIF 가 적용 전 쌍둥이(`frame-N.plain.png`)를 굽고, 없거나 `true` 인 줄은 canonical `frame-N.png`. plain 쌍둥이가 없는 런에서는 **굽기에** 무의미(플래그와 무관하게 canonical 을 굽는다) — 단 웹뷰는 이런 줄에도 **표시 전용** 온디맨드 픽셀 프리뷰 토글을 보여줄 수 있다(`ppPreviewStates`, 서버 `.pixel-preview` 캐시, v1.56.75): 화면 표시만 바꾸고 이 sidecar 필드로 저장되지 않으며 compose/export/GIF 에 영향이 없다. manifest 는 줄별 `animation.rows.<state>.frame_variant` 와 top-level 요약(`pixel`/`plain`/`mixed`)을 기록한다. 상세는 [`pixel-perfect.md`](pixel-perfect.md).
