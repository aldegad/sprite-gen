...
- `run_revision` — top-level, **required**; stamped at write by `curation.run_revision`: the
  frame generation (a fingerprint of the request + frames-manifest + each frame's
  name/size/mtime) this curation was made for. `load_curation` applies the sidecar **only when
  it matches** the current run exactly; a mismatched stamp (frames regenerated by a `--force`
  re-import or a re-extract) **and a missing stamp** (a legacy/hand-written sidecar that can't
  be verified against the current frames) are both treated as stale — the sidecar is ignored
