...
**이 정책의 범위는 추출 스냅 경로다.** 큐레이터의 베이스 격자 오버레이
(`scripts/serve_curation.py`)는 여전히 `detect_pixel_grid` 가 돌려주는 히스토그램 위상으로
격자선을 그린다 — 대상이 다른 이미지(`base-source`)이고, 픽셀을 샘플링하는 게 아니라 사람이
베이스를 편집할 때 참고할 안내선을 그리는 용도라 위 실사고(칸 병합)가 성립하지 않는다.
"히스토그램 위상은 쓰지 않는다"는 **스냅이 그것으로 칸을 자르지 않는다**는 뜻이지, 그 값을
어디서도 안 쓴다는 뜻이 아니다.
...
**7.91×**, `founder_v8` 14상태 전체 추출 **4:20 → 1:57 = 2.22×**, 출력은 프레임 PNG 204장
SHA-256 전부 동일). 전수 회귀는 68프레임 중 동일 53 / 변경 15(전부 ±1칸)였다.
