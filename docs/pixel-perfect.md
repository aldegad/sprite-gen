...
확대 표시는 항상 nearest(`image-rendering: pixelated`, 패시브) — 안티앨리어싱 확대가 실픽셀 품질을 뭉개 보이게 하는 착시를 막는다. 헤더의 **"픽셀퍼펙트 격자" 체크박스**는 픽셀퍼펙트가 실제로 스냅한 논리 픽셀 격자를 카드 위에 오버레이한다(표시 전용, 굽기 무관; 간격 = `cell_height // logical_height`). 픽셀퍼펙트가 아닌 런은 스냅 격자가 없으므로 토글이 뜨지 않는다 — 가짜 격자를 그리지 않는다.
