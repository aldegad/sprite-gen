...
확대 표시는 항상 nearest(`image-rendering: pixelated`, 패시브) — 안티앨리어싱 확대가 실픽셀 품질을 뭉개 보이게 하는 착시를 막는다. 헤더의 **"픽셀 그리드" 체크박스**는 셀 픽셀 래스터를 카드 위에 오버레이한다(표시 전용, 굽기 무관) — 픽셀이 격자에 딱 떨어지는지 눈으로 검증하는 스텝.
