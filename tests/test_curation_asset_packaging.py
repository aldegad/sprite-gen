...
def test_the_package_data_patterns_collect_every_curator_asset() -> None:
    ...
    on_disk = {f"curator/{p.relative_to(CURATOR_DIR).as_posix()}" for p in _curator_assets()}
...
def test_the_built_wheel_carries_every_curator_asset(wheel_names: set[str]) -> None:
    ...
    on_disk = {f"sprite_gen/curator/{p.relative_to(CURATOR_DIR).as_posix()}"
               for p in _curator_assets()}
    in_wheel = {n for n in wheel_names if n.startswith("sprite_gen/curator/")}
