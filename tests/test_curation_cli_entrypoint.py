...
`python -m sprite_gen.serve_curation` and the `scripts/serve_curation.py` wrapper. A copied
...
from sprite_gen import cli, serve_curation

ROOT = Path(__file__).resolve().parents[1]


def _help_options(*argv: str) -> set[str]:
    proc = subprocess.run([sys.executable, *argv, "--help"], cwd=ROOT,
...
def test_every_launch_form_exposes_the_same_flags() -> None:
    ...
    module = _help_options("-m", "sprite_gen.serve_curation")
