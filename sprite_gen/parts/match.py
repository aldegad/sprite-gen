...
from sprite_gen.parts.catalog import DEFAULT_VARIANT, job_name, jobs, load_catalog, parts_by_z

SCALE_STEPS = (0.94, 0.97, 1.0, 1.03, 1.06)
...
AGREE_FLOOR = 0.85     # fraction of a part's visible pixels that must agree for the part to pass
...
def register(part: Image.Image, base: np.ndarray, free: np.ndarray, bbox: list[int],
    ...
    Scoring happens inside a window around the bbox (not the whole canvas), so a
    1024-wide base costs milliseconds per placement instead of a full-frame pass.
    """
    ...
    def cost(x: int, y: int, w: int, h: int) -> tuple[float, float, float]:
        return score_placement(_place(trimmed, window, x - wx0, y - wy0, w, h), base_w, free_w, area)
    ...
    for scale in SCALE_STEPS:
        ...
        for dy in _offsets(ry, COARSE_STEP):
            for dx in _offsets(rx, COARSE_STEP):
                obj, colour, agree = cost(cx + dx, cy + dy, w, h)
                if best is None or obj < best["objective"]:
                    best = {"objective": obj, "score": colour, "agree": agree, "scale": scale,
                            "x": cx + dx, "y": cy + dy, "w": w, "h": h}
        assert best is not None
        for dy in range(-COARSE_STEP + 1, COARSE_STEP):
            for dx in range(-COARSE_STEP + 1, COARSE_STEP):
                obj, colour, agree = cost(best["x"] + dx, best["y"] + dy, w, h)
                if obj < best["objective"]:
                    best = {**best, "objective": obj, "score": colour, "agree": agree,
                            "x": best["x"] + dx, "y": best["y"] + dy}
...
def match_parts(catalog_path: Path, parts_dir: Path, out_dir: Path | None = None,
    ...
    for part in reversed(parts_by_z(catalog)):
        ...
        ok = bool(best["score"] <= tolerance and best["agree"] >= AGREE_FLOOR)
        record.update({"ok": ok, "placement": best, "tolerance": tolerance, "agree_floor": AGREE_FLOOR})
        if not ok:
            record["error"] = (f"best score {best['score']} exceeds tolerance {tolerance}" if best["score"] > tolerance
                               else f"only {best['agree']:.0%} of the part agrees with the base (floor {AGREE_FLOOR:.0%})")
        records.append(record)
